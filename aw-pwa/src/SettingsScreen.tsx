...
const SettingsScreen = ({ onClose }: SettingsScreenProps) => {
  const [pushToTalk, setPushToTalk] = useState(DEFAULT_SETTINGS.pushToTalk);
  const [language, setLanguage] = useState(DEFAULT_SETTINGS.language);
  const [frameRate, setFrameRate] = useState(DEFAULT_SETTINGS.frameRate);
  const [detailLevel, setDetailLevel] = useState(DEFAULT_SETTINGS.detailLevel);
  const [streamModel, setStreamModel] = useState(DEFAULT_SETTINGS.streamModel);
  const [deepChecks, setDeepChecks] = useState(DEFAULT_SETTINGS.deepChecks);
  ...
  useEffect(() => {
    try {
      ...
      setPushToTalk(persisted.pushToTalk);
      setLanguage(persisted.language);
      setFrameRate(persisted.frameRate);
      setDetailLevel(persisted.detailLevel);
      setStreamModel(persisted.streamModel);
      setDeepChecks(persisted.deepChecks);
      setThinkingLevel(persisted.thinkingLevel);
    } catch {
      setSaveStatus("error");
    } finally {
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = window.setTimeout(() => {
      try {
        ...
        saveSettingsToStorage(settingsToPersist);
        setSaveStatus("saved");
      } catch {
        setSaveStatus("error");
      }
  ...
  return (
    <>
      <Stack
        spacing={4}
        sx={{ width: "100%", pt: "10px", pb: { xs: 4, md: 6 } }}
      >
        <Stack
          direction="row"
      ...
      <Snackbar
        ...
        autoHideDuration={saveStatus === "saved" ? 1800 : 3000}
        onClose={(_, reason) => {
          if (reason === "clickaway") {
            return;
          }
